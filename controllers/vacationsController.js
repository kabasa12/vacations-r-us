...
exports.getVacations = async (req, res, next) => {
    ...
    let id = req.query.id
    try {
        vacations = await con (`SELECT v.id,v.name,v.destination,v.description,
...
exports.getVacationById = async (req, res, next) => {
    ...
    let vacation = [];
    try {
        vacation = await con (`SELECT * FROM vacations WHERE id = "${id}"`);
        //vacation = vacation[0]
    } catch (err) {
...
exports.getuserVacationById = async (req, res, next) => {
    ...
    let vacation = [];
    try {
        vacation = await con (`SELECT uv.user_id FROM vacations v 
...
exports.insertVacation = async (req, res, next) => {
    ...
    let v = [];
    try {
        v = await con ("INSERT INTO vacations (name,destination,description,image,from_date,to_date,price,num_of_followers,show_vacation) VALUES (?,?,?,?,?,?,?,?,?)",
...
exports.insertUserVacation = async (req, res, next) => {
    ...
    let v = [];
    try {
        v = await con ("INSERT INTO users_vacation (user_id,vacation_id) VALUES (?,?)",
...
exports.deleteUserVacation = async (req, res, next) => {
    ...
    let v = [];
    try {
        v = await con ("DELETE FROM users_vacation WHERE user_id = ? AND vacation_id = ?",
                              [vacation.user_id,vacation.vacation_id])
        //v = v[0];
    } catch (err) {
        v = err.message;
...
exports.deleteVacation = async (req, res, next) => {
    ...
    let v = [];
    try {
        v = await con (`DELETE FROM vacations WHERE id = ${id}`)
        //v = v[0];
    } catch (err) {
        v = err.message;
    }
