...
exports.getPermission = async (req, res, next) => {
    let permission = [];
    try {
        permission = await con (`SELECT * FROM permissions`);
...
exports.getRoles = async (req, res, next) => {
    let roles = [];
    try {
        roles = await con (`SELECT * FROM roles`);
        //roles = roles[0];
    } catch (err) {
        roles = err.message;
    }
...
exports.getRolePermission = async (req, res, next) => {
    let rolePerm = [];
    try {
        rolePerm = await con (`SELECT p.id as permissions_id,p.name as permission_name,r.name as role_name
...
exports.getRoleByUserId = async (req, res, next) => {
    ...
    let rolePerm = [];
    try {
        rolePerm = await con (`SELECT r.name  as role_name,
